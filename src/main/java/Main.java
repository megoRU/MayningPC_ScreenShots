...
public class Main {
  ...
  public static void main(String[] args) {
    for (; ; ) {
      try {
        Document doc = Jsoup.connect("https://megolox.ru/mayning/").data("query", "Java").userAgent(
        ...
        ImageIO.write(screenShot, "JPG",
            new File("src/main/resources/" + formatter.format(now.getTime()) + ".jpg"));
        System.out.println(formatter.format(now.getTime()));
        ChannelSftp channelSftp = setupJsch();
        channelSftp.connect();
        channelSftp.put("src/main/resources/" + formatter.format(now.getTime()) + ".jpg",
            folderSFTP + formatter.format(now.getTime()) + ".jpg");
        channelSftp.exit();
        Files.delete(Path.of("src/main/resources/" + formatter.format(now.getTime()) + ".jpg"));
